...
def queue_install(c, client_id: str, app_id: str):
    ...
    if not a: raise HTTPException(404, 'Aplicativo não encontrado')
    existing = v5.one(c, "SELECT * FROM base2_remote_apps WHERE client_id=? AND app_id=? AND status='pending'", (client_id, app_id))
    if not existing:
        v5.ex(c, 'INSERT INTO base2_remote_apps(id,client_id,app_id,status,created_at,result) VALUES(?,?,?,?,?,?)',(secrets.token_hex(8),client_id,app_id,'pending',v5.now(),''))
    return cl,a


def save_app(apk, name):
    fn=v5.save(apk,'apk',{'.apk'}); p=v5.UPLOAD_DIR/fn
    data=p.read_bytes(); h=hashlib.sha256(data).hexdigest(); m=v5.apkmeta(p); m['name']=name.strip() or m['name']; aid=secrets.token_hex(8)
    c=v5.db()
    try:
        v5.ex(c,'INSERT INTO apps(id,name,package_name,version_name,version_code,filename,size_bytes,sha256,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(aid,m['name'],m['package_name'],m['version_name'],m['version_code'],fn,len(data),h,v5.now()))
        v5.ex(c,'INSERT INTO base2_app_blobs(app_id,data,size_bytes,sha256,created_at) VALUES(?,?,?,?,?)',(aid,data,len(data),h,v5.now()))
        c.commit()
    except Exception:
...
def app_cards(rows):
    return ''.join(f'<div class="card"><b>{admin.esc(a["name"])}</b><br><span class="muted">{admin.esc(a.get("package_name") or "-")} • versão {admin.esc(a.get("version_name") or "-")}</span></div>' for a in rows) or '<div class="card muted">Nenhum aplicativo cadastrado.</div>'
...
def admin_app_upload(req:Request,name:str=Form(''),apk:UploadFile=File(...)):
    admin.adm(req);save_app(apk,name);return admin.redir('/base2/painel/aplicativos?ok=APK+adicionado+e+salvo')

@app.post('/base2/painel/aplicativos/instalar')
def admin_install(req:Request,client_id:str=Form(...),app_id:str=Form(...)):
    admin.adm(req);c=v5.db();cl,a=queue_install(c,client_id,app_id);c.commit();c.close();return admin.redir('/base2/painel/aplicativos?ok=Instalação+enviada')
...
def reseller_apps(req:Request,ok:str=''):
    ...
    c=v5.db();apps=v5.rows(c,'SELECT * FROM apps ORDER BY name');clients=v5.rows(c,'SELECT * FROM base2_clients WHERE reseller_id=? ORDER BY name',(r['id'],));c.close();aopts=''.join(f'<option value="{a["id"]}">{reseller.esc(a["name"])} • {reseller.esc(a.get("package_name") or "")}</option>' for a in apps);copts=''.join(f'<option value="{x["id"]}">{reseller.esc(x["name"])} • {reseller.esc(x["username"])}</option>' for x in clients);msg=f'<div class="card good">{reseller.esc(ok)}</div>' if ok else ''
    body=msg+f'''<div class="card"><h3>Adicionar APK</h3><form enctype="multipart/form-data" method="post" action="/base2/revenda/aplicativos/upload"><input name="name" placeholder="Nome opcional"><input type="file" name="apk" accept=".apk" required><button>ADICIONAR APK</button></form></div><div class="card"><h3>Instalar remotamente em cliente</h3><form method="post" action="/base2/revenda/aplicativos/instalar"><select name="client_id" required><option value="">Escolha o cliente</option>{copts}</select><select name="app_id" required><option value="">Escolha o aplicativo</option>{aopts}</select><button>INSTALAR NO CLIENTE</button></form></div><div class="grid">{app_cards(apps)}</div>''';return reseller.page('Aplicativos / Instalação remota',body)

@app.post('/base2/revenda/aplicativos/upload')
def reseller_app_upload(req:Request,name:str=Form(''),apk:UploadFile=File(...)):
    reseller.require_reseller(req);save_app(apk,name);return reseller.redir('/base2/revenda/aplicativos?ok=APK+adicionado+e+salvo')
...
def reseller_install(req:Request,client_id:str=Form(...),app_id:str=Form(...)):
    ...
    queue_install(c,client_id,app_id);c.commit();c.close();return reseller.redir('/base2/revenda/aplicativos?ok=Instalação+enviada')
...
async def api_pending_apps(req:Request):
    ...
    x=v5.one(c,"SELECT q.id queue_id,a.* FROM base2_remote_apps q JOIN apps a ON a.id=q.app_id WHERE q.client_id=? AND q.status='pending' ORDER BY q.created_at LIMIT 1",(auth['client_id'],));c.close()
    if not x:return {'ok':True,'app':None}
    return {'ok':True,'app':{'queue_id':x['queue_id'],'id':x['id'],'name':x['name'],'package_name':x.get('package_name') or '','version_name':x.get('version_name') or '','version_code':x.get('version_code') or '','download_url':f"/base2/api/apps/{x['id']}/download",'sha256':x.get('sha256') or ''}}

@app.post('/base2/api/apps/result')
async def api_remote_app_result(req:Request):
    try:d=await req.json()
    except Exception:return JSONResponse({'ok':False,'error':'invalid_json'},400)
    token=str(d.get('token') or '').strip();device=str(d.get('device_id') or '').strip();qid=str(d.get('queue_id') or '').strip();status=str(d.get('status') or 'done').strip()[:20];result=str(d.get('result') or '')[:500];c=v5.db();auth=_auth_client_by_token(c,token,device)
    if not auth:c.close();return JSONResponse({'ok':False,'error':'unauthorized'},401)
